...
-- FIXED VERSION - No reference to non-existent 'frequency' column
...
-- Copy data from 'due_date' column to 'instance_date' if needed and instance_date is null
UPDATE task_instances 
SET instance_date = COALESCE(instance_date, due_date, CURRENT_DATE)
...
-- Ensure all tasks have at least one frequency (set default for empty arrays)
UPDATE master_tasks 
SET frequencies = ARRAY['every_day']
WHERE frequencies IS NULL OR frequencies = '{}';

-- Update timing values to match new constraint (only if they don't match)
UPDATE master_tasks 
SET timing = CASE 
  ...
  WHEN timing IN ('opening', 'anytime_during_day', 'before_order_cut_off', 'closing') THEN timing
  ELSE 'opening'
END
WHERE timing IS NULL OR timing NOT IN ('opening', 'anytime_during_day', 'before_order_cut_off', 'closing');
...
-- Update task instance statuses to match new engine (only update old status names)
UPDATE task_instances 
SET status = CASE 
