-- Migration to update master_tasks table to match project specifications
-- This migration adds the missing fields and updates existing ones
...
-- Update existing data to use new structure where possible
...
-- Set default responsibility based on position_id (if we can map it)
-- This is a best-effort migration - admin should review and update as needed
UPDATE master_tasks 
...
COMMENT ON TABLE master_tasks IS 'Master tasks table updated to match project specifications with multi-select responsibility and categories';
COMMENT ON COLUMN master_tasks.responsibility IS 'Multi-select array of responsibilities (pharmacist-primary, pharmacist-supporting, pharmacy-assistants, dispensary-technicians, daa-packers, shared-exc-pharmacist, shared-inc-pharmacist, operational-managerial)';
COMMENT ON COLUMN master_tasks.categories IS 'Multi-select array of categories (stock-control, compliance, cleaning, pharmacy-services, fos-operations, dispensary-operations, general-pharmacy-operations, business-management)';
